...
class PerformanceMonitor:
    ...
    def get_efficiency_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_model_size": self.metrics_history[-1].model_size if self.metrics_history else "unknown",
            "efficiency_score": self._calculate_efficiency_score(avg_tokens_per_sec, avg_cpu, avg_memory_percent)
        }
    
    def _calculate_efficiency_score(self, tokens_per_sec: float, cpu_percent: float, memory_percent: float) -> float:
    ...
    def get_optimization_recommendations(self) -> list[str]:
        ...
        if report["average_cpu_percent"] > 85:
            recommendations.append("CPU usage is high. Consider reducing thread count for better thermal performance.")
        
        # Check if memory usage is too high
        if report["average_memory_percent"] > 80:
            recommendations.append("Memory usage is high. Consider using smaller models or reducing context length.")
        
        # Check if efficiency score is low
        if report["efficiency_score"] < 60:
            recommendations.append("Overall efficiency could be improved. Consider using the 1.5B model for better performance on lightweight devices.")
        
        # Check if tokens per second is low
        if report["average_tokens_per_second"] < 5:
            recommendations.append("Generation speed is slow. Ensure BLAS acceleration is enabled and appropriate thread count is set.")
        
        if not recommendations:
            recommendations.append("Performance looks good! System is running efficiently on lightweight hardware.")
