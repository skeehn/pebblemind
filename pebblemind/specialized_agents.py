...
class ResearchAgent(BaseAgent):
    ...
    async def execute(
        self, task: str, search_query: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        ...
        wiki_result = await tool_manager.execute_tool(
            "wikipedia",
            {
                "query": (
        ...
        )  # Use first word for Wikipedia

        # Synthesize results
        synthesis = self._synthesize_results(
            search_result.get("result", []), wiki_result.get("result", "")
        )
        ...
        if search_results:
            ...
            for i, result in enumerate(search_results[:3]):  # Take top 3 results
                summary_parts.append(
                    f"{i+1}. {result.get('title', 'No title')}: {result.get('snippet', 'No snippet')[:100]}..."
                )
...
class CodeAgent(BaseAgent):
    ...
    async def execute(
        self, task: str, code: str = "", language: str = "python", **kwargs
    ) -> Dict[str, Any]:
        ...
        if code:
            ...
            if language.lower() == "python":
                exec_result = await tool_manager.execute_tool(
                    "code_executor", {"language": language, "code": code}
                )
                result_parts.append(f"Execution result: {exec_result}")

            # Add code explanation
            result_parts.append(
                f"Code analysis for {language} code: {self._explain_code(code, language)}"
            )
...
class MathAgent(BaseAgent):
    ...
    async def execute(
        self, task: str, expression: str = "", **kwargs
    ) -> Dict[str, Any]:
        ...
        if expression:
            ...
            result_parts.append(
                f"Calculation result: {calc_result.get('result', calc_result.get('error', 'Could not calculate'))}"
            )
    ...
    def _solve_math_problem(self, task: str) -> str:
        ...
        if any(word in task.lower() for word in ["solve", "find", "calculate"]):
            ...
            return (
                "Rearrange equation to isolate the variable, then solve step by step."
            )
        else:
            return (
                "Break the problem into smaller parts, identify what is being asked, "
                "and apply mathematical principles."
            )
...
class WritingAgent(BaseAgent):
    ...
    def _process_text(self, text: str, task: str) -> str:
        ...
        if "summarize" in task.lower() or "summary" in task.lower():
            ...
            if len(words) > 50:  # Only summarize if it's long enough
                return (
                    " ".join(words[: min(50, len(words))]) + " [truncated for summary]"
                )
            else:
                return text
    ...
    def _provide_writing_help(self, task: str) -> str:
        ...
        if "outline" in task.lower():
            return (
                "Start with main points, add supporting details, ensure logical flow."
            )
...
class AgentOrchestrator:
    ...
    async def route_task(self, task: str) -> Dict[str, Any]:
        ...
        if any(
            ...
            word in task_lower
            for word in ["calculate", "math", "equation", "solve", "formula", "compute"]
        ):
            ...
            if any(
                word in task_lower for word in ["data", "analyze", "study", "learn"]
            ):
                agent = self.agents["research"]
            elif any(
                char in task_lower for char in ["=", "calculate", "+", "-", "*", "/"]
            ):
                agent = self.agents["math"]
            else:
