...
class ImageProcessor:
    ...
    def __init__(self):
        ...
        if not NUMPY_AVAILABLE:
            raise ImportError(
                "NumPy is required for image processing. Install with: pip install numpy"
            )
...
class MultiModalManager:
    ...
    async def process_multimodal_input(
        ...
        if image_data and self.image_processor:
            ...
            if text:
                results["combined_analysis"] = await self._create_combined_analysis(
                    text, image_result
                )
    ...
    async def _create_combined_analysis(
        self, text: str, image_analysis: Dict[str, Any]
    ) -> str:
        """Create a combined analysis from text and image data"""
        image_desc = image_analysis.get("description", "image properties")
        relation = image_desc.split("with")[1] if "with" in image_desc else image_desc
        return f"Text: {text}\nImage: {image_desc}\nCombined: The text appears to relate to the {relation}"
...
async def process_image_description(
    image_data: Union[str, bytes], multimodal_manager: MultiModalManager
) -> str:
    ...
    result = await multimodal_manager.process_multimodal_input(
        image_data=image_data, operation="describe"
    )
