...
class ReasoningEnhancer:
    ...
    def enhance_prompt(
        ...
        if use_chain_of_thought:
            ...
            chain_text = "Please think through this step by step:\n" + "\n".join(
                [f"{i+1}. {step}" for i, step in enumerate(chain_template)]
            )
    ...
    async def apply_reasoning_pipeline(
        self,
        query: str,
    ...
    def _get_reasoning_type_from_response(self, response: str) -> ReasoningType:
        ...
        if any(
            ...
            for word in ["analyze", "break down", "components", "parts"]
        ):
            return ReasoningType.ANALYTICAL
        elif any(
            word in response_lower
            for word in ["solve", "solution", "approach", "method"]
        ):
            return ReasoningType.PROBLEM_SOLVING
        elif any(
            word in response_lower
            for word in ["logical", "reason", "follows", "implies"]
        ):
            return ReasoningType.LOGICAL
        elif any(
            word in response_lower
            for word in ["alternative", "creative", "innovative", "think outside"]
        ):
