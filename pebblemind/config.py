...
class LLMConfig(BaseModel):
    ...
    backend: str = Field(
        default="local",
        description="Backend type: 'local' (llama.cpp) or 'groq' (cloud API)",
    )
    model_path: str = Field(
        default="", description="Path to the LLM model file (auto-detected if empty)"
    )
    model_name: str = Field(default="Qwen2.5-1.5B-Instruct", description="Model name")
    model_size: str = Field(
        default="1.5b",
        description="Model size: 1.5b (ultra-light, MacBook Air optimized), 3b (balanced), 7b (high-quality)",
    )
    context_length: int = Field(
        default=2048,
        description="Maximum context length (optimized for lightweight devices)",
    )
    max_tokens: int = Field(
        default=256,
        description="Maximum tokens to generate (conservative for efficiency)",
    )
    ...
    threads: int = Field(
        default=-1,
        description="Number of threads (-1 for auto, conservative for lightweight devices)",
    )
    batch_size: int = Field(
        default=256,
        description="Batch size for processing (reduced for memory efficiency)",
    )
    ...
    enable_gpu_offload: bool = Field(
        default=False,
        description="Enable GPU layer offloading (CPU-only by default for consistency)",
    )
    gpu_layers: int = Field(
        default=0, description="Number of layers to offload to GPU (-1 for auto)"
    )
    auto_detect_gpu: bool = Field(
        default=True, description="Automatically detect and configure GPU"
    )
...
class GroqConfig(BaseModel):
    ...
    api_key: Optional[str] = Field(
        default=None, description="Groq API key (or set GROQ_API_KEY env var)"
    )
    model: str = Field(
        default="llama-3.1-70b-versatile", description="Groq model to use"
    )
    base_url: str = Field(
...
class VoiceConfig(BaseModel):
    ...
    stt_model: str = Field(
        default="base.en", description="Whisper model for speech-to-text"
    )
...
class RAGConfig(BaseModel):
    ...
    embedding_model: str = Field(
        default="BAAI/bge-small-en-v1.5", description="Lightweight embedding model"
    )
    embedding_dim: int = Field(default=384, description="Embedding dimension")
    vector_db_path: str = Field(
        default="./data/vectors.db", description="Vector database path"
    )
    chunk_size: int = Field(default=512, description="Document chunk size")
    chunk_overlap: int = Field(
        default=32, description="Overlap between chunks (reduced for efficiency)"
    )
    max_results: int = Field(
        default=3, description="Maximum search results (reduced for efficiency)"
    )
