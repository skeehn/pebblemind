...
class LongTermMemory:
    ...
    def _init_db(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_type_timestamp ON memories(memory_type, timestamp)"
        )
        cursor.execute(
    ...
    async def consolidate_memories(self, consolidation_period: int = 7) -> int:
        """
...
class EnhancedMemoryManager:
    ...
    async def store_factual_memory(
        self, fact: str, importance: float = 0.8, tags: List[str] = None
    ) -> bool:
    ...
    async def retrieve_relevant_context(
        self, query: str, max_memories: int = 5, include_types: List[str] = None
    ) -> List[str]:
        ...
        memories = await self.long_term_memory.retrieve_memories(
            query=query, memory_type=None, limit=max_memories
        )

        # Filter by requested types
        filtered_memories = [
            mem for mem in memories if mem.memory_type in include_types
        ]
    ...
    async def get_memory_summary(self) -> str:
        ...
        if stats["oldest_memory"]:
            summary_parts.append(
                f"Oldest: {stats['oldest_memory'].strftime('%Y-%m-%d')}"
            )

        if stats["newest_memory"]:
            summary_parts.append(
                f"Newest: {stats['newest_memory'].strftime('%Y-%m-%d')}"
            )
