...
class EfficientReasoningEngine:
    ...
    async def initialize(self, device_type: str = "macbook_air") -> None:
        ...
        try:
            logger.info("Initializing Efficient Reasoning Engine for lightweight devices...")
            
            # Apply device-specific optimizations
            if device_type.lower() == "macbook_air":
                # MacBook Air specific optimizations
                self.config.context_length = self._mac_optimized_params["context_length"]
            ...
            logger.info(f"Loading optimized model: {self.config.model_name} ({self.config.model_size})")
            ...
            logger.info(f"Efficient Reasoning Engine initialized successfully on {device_type}")
            logger.info(f"Model loaded with {self.config.threads} threads and {self.config.context_length} context length")
    ...
    def _validate_model_path(self, model_path: str) -> bool:
        ...
        if size_gb > max_size_gb:
            logger.warning(f"Model file is larger than recommended for lightweight devices: {size_gb:.2f} GB")
    ...
    async def generate(
        ...
        **kwargs
        ...
        try:
            ...
            generation_params = {
                "messages": messages,
                "max_tokens": max_tokens or min(256, self.config.max_tokens),  # Reduced for efficiency
                "temperature": temperature or min(0.7, self.config.temperature),  # Conservative for quality
            ...
            response = await loop.run_in_executor(None, lambda: self.model.create_chat_completion(**generation_params))
    ...
    async def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_tokens_per_second": self._performance_stats["average_tokens_per_second"],
