...
class ExternalServiceConnector:
    ...
    async def connect_to_database(
        self, db_type: str, connection_string: str
    ) -> Dict[str, Any]:
    ...
    async def query_database(
        self, db_type: str, query: str, connection_string: str = None
    ) -> Dict[str, Any]:
    ...
    async def connect_to_weather_service(self, api_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "service": "weather", "error": str(e)}
...
class ServiceIntegrationManager:
    ...
    async def integrate_with_service(
        ...
        if service_type.lower() == "database":
            ...
            result = await self.service_connector.connect_to_database(
            ...
            result = await self.service_connector.call_rest_api(
    ...
    async def execute_external_query(self, query_type: str, **kwargs) -> Dict[str, Any]:
        ...
        if query_type.lower() == "database_query":
            ...
            return await self.service_connector.query_database(
                db_type, query, connection_string
            )
            ...
            if not url:
                return {"status": "error", "error": "url is required for REST calls"}

            return await self.service_connector.call_rest_api(
                url, method, headers, params, data
            )
...
async def execute_external_task(
    task: str, service_manager: ServiceIntegrationManager, config: Dict[str, Any]
) -> str:
